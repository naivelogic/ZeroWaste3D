...
class MaskManager:
    def __init__(self, dataset_path):  # dataset_config_path):
        ...
        # used to add a experimental custom class (e.g., instead of fork/spoon all are utensels)
        self.custom_classes_flag = False
        ...
        self.dataset_config_path = os.path.join(
        ...
        self.categories = ["H_beveragebottle", "D_lid", "S_cup"]
    ...
    #def set_resave_mask(self, new_mask_path='/home/redne/ZeroWaste3D/DataManager/create_dataset/sample_maya_raw/ds1/masks/'):
    def set_resave_mask(self, new_mask_path='/home/redne/ZeroWaste3D/DataManager/create_dataset/sample_maya_raw/ds1/masks/'):
        self.resave_masks = True
    ...
    def parseInstancelabels(self, path):
        ...
        with open(path) as inJsonText:
            ...
            if "InstanceUniqueIds" not in inJson['Events'][0]['Event']['Data']:
                raise RuntimeError(
                    'BAD JSON; This is not an instance GT folder')
            ...
            for dataArray in inJson['Events'][0]['Event']['Data']["Arr"]:
                ...
                instanceMetadataDictionaryByName[instanceMetadata['Name']
                                                 ] = instanceMetadata
    ...
    def resave_mask_png(self, instanceImagePath, instanceLabels, instance_name):
        ...
        """
        instanceImage = cv2.imread(
            instanceImagePath, cv2.IMREAD_ANYCOLOR | cv2.IMREAD_ANYDEPTH)
        ...
        outMaskImageForViewing = self.createColorMaskImage(
            instanceImage, instanceLabels, 'CategoryPath', self.colorMapping)

        outMask_path = os.path.join(
            self.resave_mask_path, instance_name+"_mask_.png")
    ...
    def old_make_datapath_list(self, path, sample_amount=0.1):
        ...
        train_, val_ = mask_paths[-masklen +
                                  sample:], mask_paths[:-masklen+sample]
    ...
    def start(self, phase, mask_paths):
        ...
        raw = '/home/redne/ZeroWaste3D/DataManager/create_dataset/sample_maya_raw/ds1/raw'
        ...
        for instance_ in mask_paths:
            ...
            if self.resave_images_flag == True:
                rgbImage = cv2.imread(
                    rgb_path, cv2.IMREAD_ANYCOLOR | cv2.IMREAD_ANYDEPTH)
                rgb_path = os.path.join(
                    self.resave_images_path, instance_name + "_rbg.jpg")
            ...
            mask_event = os.path.join(
                self.dataset_raw_folder, instance_name, 'instance/events.0.json')

            #instanceImagePath = raw + instance_name_raw + '/instance/outputs'
            #instanceImagePath = glob.glob(instanceImagePath + "/*.exr")[0]

            instanceImagePath = os.path.join(
                self.dataset_raw_folder, instance_name, "instance/outputs")
            instanceImagePath = glob.glob(instanceImagePath + "/*.exr")[0]
            ...
            if self.resave_masks == True:
                ...
                outMaskImageForViewing = createColorMaskImage(
                    instanceImage, instanceLabels, 'CategoryPath', self.colorMapping)
                mask_path = os.path.join(
                    self.resave_mask_path, instance_name+"_mask.png")
                # uncomment if new dataset
    ...
    def write_masks_to_json(self, phase):
        ...
        output_file_path = self.dataset_config_path + \
            f'{phase}_mask_definitions.json'
